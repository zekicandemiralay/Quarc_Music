...
3. Share `https://quarcnet0.tail84500c.ts.net:4000` and their credentials
4. They need Tailscale installed and connected — direct them to **README_Users.md**
5. Android users can also install the native APK from the [latest release](https://github.com/zekicandemiralay/Quarc_Music/releases/latest)
...
| `VPN_COUNTRY` | `Switzerland` | VPN server country for downloads — must be one the provider/plan has. Paid Proton: any country; free tier is limited to Canada, Japan, Mexico, Netherlands, Norway, Poland, Romania, Singapore, Switzerland, US. Prefer one near the server, since every download crosses this tunnel |
