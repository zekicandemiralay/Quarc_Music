...
export default function Library({ view = 'all' }) {
  ...
  // Swipe-right-to-queue tracking (mobile)
  ...
  // Non-passive touchmove on the list container to detect horizontal swipes
  useEffect(() => {
    const el = swipeListRef.current;
    if (!el) return;
    const handler = (e) => {
      ...
      if (!sr.isH) {
        if (Math.abs(dy) > 8) { sr.song = null; return; } // vertical — let browser scroll
        if (Math.abs(dx) > 8) {
          if (dx > 0) sr.isH = true; // rightward — start swipe
          else { sr.song = null; return; } // leftward — abort
        }
        return;
      }
      if (dx > 0) {
        e.preventDefault();
        sr.lastDx = dx;
        if (sr.el) sr.el.style.transform = `translateX(${Math.min(dx, 100)}px)`;
      }
    };
    el.addEventListener('touchmove', handler, { passive: false });
    return () => el.removeEventListener('touchmove', handler);
