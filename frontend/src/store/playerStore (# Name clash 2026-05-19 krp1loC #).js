...
// iOS can pause audio without firing the 'pause' DOM event (e.g. after lock screen).
// When the app becomes visible again, sync real audio state back to the store and
// retry playback if the user had pressed play on the lock screen.
document.addEventListener('visibilitychange', () => {
  ...
  if (isPlaying && audio.paused) {
    if (audio.readyState < 2 && currentSong) {
      // Buffer gone — reload stream and resume from last known position
      const t = audio.currentTime || storeTime;
      audio.src = `/api/music/${currentSong.id}/stream`;
      audio.addEventListener('canplay', () => {
        if (t > 0) audio.currentTime = t;
